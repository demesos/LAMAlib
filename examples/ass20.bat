...
if "%2"=="" (
  ...
  >nul findstr /c:"makesys" %1 && (
    @echo on
    cl65 "%1" -lib LAMAlib.lib -t vic20 -C vic20-asm.cfg -Ln "labels.txt" -o "%~n1.prg"
  ) || (
    @echo on
    cl65 "%1" -lib LAMAlib.lib -t vic20 -C vic20-asm.cfg -Ln "labels.txt" -u __EXEHDR__ -o "%~n1.prg"
  )
) else (
  echo assembling %1 to start address %2 for target C64...
  @echo on
  cl65 "%1" -lib LAMAlib.lib -t vic20 -C vic20-asm.cfg -Ln "labels.txt" --start-addr %2 -o "%~n1.prg"
)
@echo done.
